...
def print_annotated_graph(graph):
    for i, node in enumerate(graph.nodes):
        shape = ','.join(str(x) for x in node.meta['output_shape'])
        if node.op == 'placeholder':
            print(f"{i} {node.name}: {shape} = placeholder({node.target})")
            continue
        if node.op == 'get_attr':
            print(f"{i} {node.name}: {shape} = {node.target}")
            continue
        if node.op in ('call_function', 'call_method'):
            fname = node.target if isinstance(node.target, str) else node.target.__name__
            print(f"{i} {node.name}: {shape} = {fname}({', '.join(str(x) for x in node.args)}, {', '.join(name+'='+str(value) for name, value in node.kwargs.items())}) ")
            continue
        if node.op == 'output':
            print(f"{i} return {node.args[0]}")
            continue
